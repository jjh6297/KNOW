...
for trial in range(0,5):
    ...
    model.load_weights('KNOWN_Predicted_Recurrence_2.h5', skip_mismatch=True, by_name=True)
    ...
    sio.savemat('Save_KNOWN_Finetuned_Trial'+str(trial)+'_Recurrence_2.mat', mdict = {'Loss':np.array(hist.history['loss']), 'ValLoss':np.array(hist.history['val_loss']), 'Acc':np.array(hist.history['accuracy']), 'ValAcc':np.array(hist.history['val_accuracy'])})
    model.save_weights('Weights_KNOWN_Trial_'+str(trial)+'_Recurrence_2.h5')
